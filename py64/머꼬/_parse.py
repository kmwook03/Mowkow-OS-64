...
from _error import *
import mowio
from _compat import readline, to_int, token_prefix
...
def slurp(path: str) -> str:
    return mowio.readfile(path)     # 이식: io.open 대신 (mowkow_porting.md 5단계)
...
class Reader:
    ...
    def next_token(self) -> str:        # returns the token and set it to LA
        ...
        while s != "":
            ...
            if s.startswith(",@"):      # ",@" should be tested before ","
                ...
                tok = token_prefix(s)       # 이식: re 없음 (6단계)
            tlen = len(tok)
            self._input = s[tlen:]
    ...
    def read(self) -> str:
        try:
            self._input = readline(self._prompt1)
        except EOFError:
            return ""
        else:
            return self._input
    def read2(self) -> str:
        try:
            self._input = readline(self._prompt2 + self.indent())
        except EOFError:
            return ""
        else:
            return self._input
...
def read_atom(s: str) -> Data:
    ...
    if s.isdigit() or (len(s) > 1 and (s[0] == '+' or s[0] == '-') and s[1:].isdigit()):
        ...
        return mkint(to_int(new_lit, 16))
    elif s == '공':
        return nil
    elif s[0] == '"':
        return mkstr(s)
