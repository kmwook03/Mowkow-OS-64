#ifndef MOWKOW64_ASMFUNC64_H
#define MOWKOW64_ASMFUNC64_H

#include <stdint.h>

void io_hlt(void);
void io_cli(void);
void io_sti(void);
void io_stihlt(void);
void io_out8(uint16_t port, uint8_t data);
uint8_t io_in8(uint16_t port);
uint64_t io_load_rflags(void);
void io_store_rflags(uint64_t rflags);
void load_gdtr64(uint16_t limit, uintptr_t addr);
void load_idtr64(uint16_t limit, uintptr_t addr);
void load_tr64(uint16_t selector);
void reload_segments64(uint16_t code_selector, uint16_t data_selector);
void asm_exception_default(void);
int enter_user_mode64(uintptr_t entry, uintptr_t stack, uint64_t argc, uintptr_t argv,
	uint16_t code_selector, uint16_t data_selector, uintptr_t *saved_kernel_rsp);
void leave_user_mode64(uintptr_t kernel_rsp, int status);

#endif
