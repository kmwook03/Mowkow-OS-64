 ...
 *
 * 머꼬가 CPython 표준 라이브러리 대신 쓰는 창구다.
 ...
 *
 * MicroPython의 input()을 대신한다. 
 * input()은 repl 큐에서 바이트를 받아 스스로 echo하는데, 
 * 그 길에는 한글 오토마타가 없고 백스페이스도 바이트 단위라 3바이트 음절을 깨뜨린다. 
 * 대신 커널 콘솔의 줄 편집기를 그대로 불러 조합 중인 음절이 화면에서 자라는 것도 함께 처리한다.
 *
 * Ctrl-C는 KeyboardInterrupt, Ctrl-D는 EOFError로 올라간다.
 */
 ...
 *
 * io.open() 대신이다(MICROPY_PY_IO는 꺼져 있다). 
 * 머꼬의 slurp가 소스와 library_kor.scm을 읽는다.
 *
 * UTF-8 검사는 mp_obj_new_str에서 수행하므로 
 * 잘린 파일은 인덱싱이 어긋난 str이 되어 조용히 잘못 파싱되지 않는다.
 */
 ...
 * mowio.argv() -> str | None
 *
 * 콘솔이 `머꼬 add.mk`로 넘긴 파일 이름. 
 * 인자가 없으면(그냥 `머꼬`) None이고, MOWKOW.PY는 그걸 보고 파일 모드와 REPL을 가른다.
 */
