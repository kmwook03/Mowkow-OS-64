/* ATA PIO transport. Primary channel, master drive, LBA28, one sector per
   command. The fallback when no AHCI controller answers. */
#include <asmfunc64.h>
#include <block64.h>
#include <stddef.h>
#include <stdint.h>
...
static uint64_t ata64_sector_count(void)
{
	...
	/* ponytail: LBA28 capacity only (words 60-61); read/write use LBA28
	   commands anyway, so the 48-bit fields would be unreachable. */
