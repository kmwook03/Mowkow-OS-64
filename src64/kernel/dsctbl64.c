/*
 * dsctbl64.c -- GDT, IDT, TSS
 *
 * 롱 모드에서는 세그먼트가 거의 사라지지만 GDT는 남아 있어야 한다. 코드/
 * 데이터 셀렉터와 링 3용 셀렉터, 그리고 인터럽트가 링 0으로 들어올 때 쓸
 * 커널 스택(TSS의 rsp0)이 여기서 정해진다.
 *
 * 32비트 트리와 달리 TSS로 태스크를 바꾸지 않는다. 롱 모드에는 하드웨어
 * 태스크 전환이 없어서 mtask64.c가 소프트웨어로 처리한다.
 */
#include <asmfunc64.h>
#include <dsctbl64.h>
