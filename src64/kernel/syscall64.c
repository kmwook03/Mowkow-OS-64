...
static int syscall_open(struct PROCESS64 *process, const char *path)
{
	size_t i;
	...
	for (i = 3; i < PROCESS64_MAX_FILES; i++) {
		if (process->files[i].used == 0) {
			if (fd64_open(&process->files[i].fh, path) == 0) {
				return -2;
			}
...
uint64_t syscall_handler64(struct INTERRUPT_FRAME64 *frame)
{
	...
	if (nr == SYS_WRITE) {
		if ((a0 == 1 || a0 == 2) && process64_user_range_valid((const void *) a1, (size_t) a2) != 0) {
	...
	if (nr == SYS_OPEN) {
		frame->rax = (uint64_t) syscall_open(process, (const char *) a0);
		return 0;
	}
	if (nr == SYS_READ) {
