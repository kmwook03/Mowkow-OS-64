...
> ⚠️ **현재 프로젝트 진행 방향: 64비트 확장/포팅**
> 이 README는 안정된 참조 구현인 **32비트** 트리(`src/`, `app/`)를 설명합니다.
> 현재 활발히 개발 중인 작업은 `src64/`, `app64/`의 **64비트 롱모드 포트**입니다.
> 64비트 작업 시 `CLAUDE.md`, `porting.md`, `abi_plan.md`, `python_porting.md`, `hangulplan.md`를 먼저 확인하세요. 64비트 기능은 `src64/`/`app64/`에만 추가하고, 32비트 트리에는 병합하지 마세요.
