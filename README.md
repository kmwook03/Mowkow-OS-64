...
| 스크립트 언어 | 머꼬 (C) | MicroPython v1.28 (커널에 링크) |
...
두 값은 함께 움직여야 합니다(`memory64.h`와 `elf64_loader.c`).
...
그러나 롱 모드에는 그 수단이 없고 페이징 격리도 아직 없습니다. 
대신 커널이 유저 포인터를 직접 검사하여 영역 침범을 방지합니다.
...
`syscall64.c`의 모든 포인터 인자가 이 검사를 지나갑니다. 문자열은
`strn_len_user`가 바이트마다 검사하며 길이를 잽니다. 향후 페이징 격리를 업데이트할 예정입니다.
...
* 커널 크기 상한은 `mk/x86_64.mk`의 `KERNEL64_SECTORS`(현재 800섹터)입니다. 넘으면 링크 직후 빌드가 멈춥니다.
...
| MicroPython 단일 인스턴스 | `mpport/` | GC 힙과 상태가 전역 하나. 콘솔마다 파이썬을 돌릴 수 없음 |
| 자동 테스트 없음 | 양쪽 트리 | 부팅 시 COM1 자체 점검과 QEMU 수동 확인이 검증의 전부 |
