...
[Mowkow OS](https://github.com/yonghun8343/Mowkow-OS)는 저서 『OS 구조와 원리』를 기반으로 제작된 교육용 한글 운영체제입니다.
...
내장 명령이 아닌 낱말은 FAT32에서 같은 이름의 실행 파일을 찾고, 없으면 `알 수 없는 명령어`라고 알립니다.
...
바탕화면 아이콘으로도 새 콘솔을 띄울 수 있고, 작업 표시줄로 숨긴 콘솔을 되살립니다.
...
한 번 머꼬 실행 환경을 생성한 후로는 `library_kor.scm`을 다시 읽지 않고 두 번째부터는 바로 뜹니다.
...
### 4-2. 메모리 배치
32bit에서 하드코딩 되어 있던 주소를 64bit는 정책선 세 개로 줄였습니다.
...
### 4-4. 유저 프로그램 ABI
32bit 앱은 `.hrb` 형식이었고, 태스크마다 만든 LDT 세그먼트의 베이스와 리밋이 앱을 커널로부터 격리 하였습니다.
그러나 롱 모드에는 그 수단이 없고 페이징 격리도 아직 없기 때문에 커널이 유저 포인터를 직접 검사하여 영역 침범을 방지합니다.
(향후 페이징 격리를 업데이트할 예정입니다.)
...
다중 콘솔을 지원하기 위해 콘솔의 상태를 `struct CONSOLE64`에 저장하고, 콘솔마다 태스크와 256KiB스택과 키 FIFO를 갖습니다. 콘솔은 한 번에 최대 4개 띄울 수 있습니다.

* 숨긴 콘솔도 계속 실행되고, GUI는 반응합니다.
* 바탕화면 아이콘이 새 콘솔을 띄우고, 작업 표시줄이 숨긴 콘솔을 되살립니다.
* 전체 화면 토글(`창`/`window`)은 콘솔 0 전용입니다. 화면 크기 버퍼를 가진 것이 콘솔 0뿐입니다.

키 라우팅은 커널 이벤트 루프가 먼저 창 계층에 넘기고(마우스와 F11은 창이 가져감), 남은 키만 포커스가 있는 콘솔의 태스크로 보냅니다. 콘솔이 아닌 창이 활성이면 키는 아무 데도 가지 않습니다.

동시성은 타이머 인터럽트가 유일한 선점 원인이므로 `io_cli`/`io_sti` 임계 구역으로 충분하고, 트리에 뮤텍스가 없으며 필요하지 않다는 판단하여 Giant Lock 하나로 처리하였습니다. 대신 임계 구역 안에서 `task_sleep64`를 부르지 않도록 합니다.

### 4-8. 머꼬 인터프리터
64bit 머꼬 OS는 원본 파이썬 소스를 커널에 링크된 MicroPython에서 실행시킵니다.

```
콘솔: 머꼬 [파일]
  └ mpport_run_mowkow()          "import mowkow; mowkow.main()"을 컴파일
      └ mowkow.py                진입점 (업스트림 main.py 대신)
          ├ base_env()           내장 함수 + library_kor.scm, 부팅당 한 번
...
| `argv()` | `sys.argv` | 문자열 파싱만을 위해 sys 라이브러리를 가져오는 것은 너무 비싼 작업입니다. |
...
명령이 끝날 때마다 MicroPython을 정리하면 `머꼬`를 부를 때마다 준비 시간 1.4초가 걸리므로 정리(`mp_deinit()`) 하지 않고 해석기를 부팅 내내 켜 둡니다. 준비된 환경이 파이썬 모듈 안에 그대로 남아 있기 때문에, 첫 `머꼬` 실행에만 1.4초가 걸리고 두 번째 실행 부터는 곧바로 실행됩니다.
