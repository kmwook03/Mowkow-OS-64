...
/* 창 크기와 위치는 화면 비례다. 높이와 위치는 800x600일 때 예전의 고정값
   (400, (64,80))과 같은 값이 나오도록 고른 비율이고, 너비만 그 4/5에서
   다시 70%로 줄인 14/25다. */
#define GUI64_WIN_W_NUM 14
#define GUI64_WIN_W_DEN 25
#define GUI64_WIN_H_NUM 2
#define GUI64_WIN_H_DEN 3
#define GUI64_WIN_X_NUM 2
#define GUI64_WIN_X_DEN 25
#define GUI64_WIN_Y_NUM 2
#define GUI64_WIN_Y_DEN 15
...
static int32_t gui_win_w;
static int32_t gui_win_h;
static int32_t gui_win_x;
...
struct SHEET64 *gui64_init(const struct BOOTINFO64 *boot_info)
{
	...
	gui_scrnx = xsize;
	gui_scrny = ysize;
	/* 글꼴 폭이 8이라 창 너비도 8의 배수로 맞춘다. */
	gui_win_w = (xsize * GUI64_WIN_W_NUM / GUI64_WIN_W_DEN) & ~7;
	gui_win_h = (ysize * GUI64_WIN_H_NUM / GUI64_WIN_H_DEN) & ~7;
	gui_win_x = xsize * GUI64_WIN_X_NUM / GUI64_WIN_X_DEN;
	gui_win_y = ysize * GUI64_WIN_Y_NUM / GUI64_WIN_Y_DEN;
...
void gui64_toggle_window(void)
{
	...
	if (gui_mode == GUI64_MODE_FULL) {
		sheet64_setbuf(gui_console, gui_console_buf, gui_win_w, gui_win_h, -1);
		make_window64(gui_console_buf, gui_win_w, gui_win_h, "터미널", 1);
		boxfill64(gui_console_buf, gui_win_w, COL64_000000,
			GUI64_PAD_X, GUI64_PAD_Y,
			gui_win_w - GUI64_PAD_W + GUI64_PAD_X - 1,
			gui_win_h - GUI64_PAD_H + GUI64_PAD_Y - 1);
		gui_console->vx0 = gui_win_x;
		gui_console->vy0 = gui_win_y;
		gui_mode = GUI64_MODE_WINDOW;
		win->decorated = 1;
		taskbar_show(1);
		console64_attach_sheet(win->console, gui_console,
			GUI64_PAD_X, GUI64_PAD_Y,
			gui_win_w - GUI64_PAD_W, gui_win_h - GUI64_PAD_H);
	} else {
...
int gui64_open_console_window(struct CONSOLE64 *con, const char *title)
{
	...
	buf = memman64_alloc_4k(&memman64, (size_t) gui_win_w * (size_t) gui_win_h);
	if (buf == 0) {
		return -1;
	}
	...
	if (sht == NULL) {
		memman64_free_4k(&memman64, buf,
			(size_t) gui_win_w * (size_t) gui_win_h);
		return -1;
	}
	sheet64_setbuf(sht, (uint8_t *) buf, gui_win_w, gui_win_h, -1);
	make_window64((uint8_t *) buf, gui_win_w, gui_win_h, title, 1);
	boxfill64((uint8_t *) buf, gui_win_w, COL64_000000,
		GUI64_PAD_X, GUI64_PAD_Y,
		gui_win_w - GUI64_PAD_W + GUI64_PAD_X - 1,
		gui_win_h - GUI64_PAD_H + GUI64_PAD_Y - 1);
	...
	sht->vx0 = gui_win_x + step;
	sht->vy0 = gui_win_y + step;
