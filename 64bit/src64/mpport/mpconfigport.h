...
/*
 * 아직 파일 시스템을 쓰는 reader가 없다. 스크립트 실행은 업스트림 코드 대신
 * fd64로 읽어 오는 자체 mp_reader를 쓴다.
 */
 ...
 * 터진다). 머꼬는 math를 쓰지 않으므로 모듈째 끈다. 필요해지면 그때 libm을 채운다.
 */
...
/*
 * 런타임 도우미가 필요 없는 binary.c의 소프트웨어 변환 경로로 우회한다.
 */
...
/*
 * EXTRA 등급이지만 따로 켠다. 업스트림 머꼬 네 모듈에 f-string이
 * 스물세 군데 있다(오류 메시지와 __str__이 거의 다 f-string이다). 끄고 가려면
 * 그 스물세 줄을 다 고쳐야 하는데, 그건 핵심 로직 수정에 해당하므로 OS 쪽을 수정한다.
 */
#define MICROPY_PY_FSTRINGS (1)

/*
 * EXTRA에서 따로 하나 켠다. _data.py:134가 내장 함수의 이름을 찍는데
 * (#<내장 함수: 머>), 함수 객체에서 이름을 꺼낼 방법이 이것뿐이다. 없으면
 * __name__ 자체가 없어 이름 자리가 "?"가 된다.
 */
