# ---------------------------------------------------------------------------
# 64비트 트리(64bit/src64, 64bit/app64) 빌드 규칙
#
...
#
# 커널에 링크할 MicroPython 오브젝트 목록(MPY_*)은 64bit/mk/micropython.mk에서
# 온다. 규칙을 읽는 시점에 그 변수들이 있어야 하므로 include 순서가 중요하다.
# ---------------------------------------------------------------------------

# -- FAT32 배치 --
# 배치는 64bit/tools/mkfat32_64.py 한 곳에만 있고, 이 makefile과 boot64.asm의 BPB가
# 거기서 값을 받아 간다. 이미지와 부트 섹터가 조용히 어긋나면 실행 중에는
# 되돌릴 방법이 없다.
MKFAT32 = $(TOOL64PATH)/mkfat32_64.py
...
$(BOOT64_BIN) : $(SRC64_DIR)/boot/boot64.asm Makefile 64bit/mk/x86_64.mk $(MKFAT32)
	@$(MKDIR) $(dir $@)
